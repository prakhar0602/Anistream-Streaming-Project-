...
const Template_4 = (props) => {
  ...
  return (
    <div className='bg-[#573087] min-w-[358.4px] flex lg:flex-row  items-center p-4 flex-col lg:p-8  rounded-xl gap-8'>
        <img src={props.series.big_image} className='lg:w-[540px] w-[300px] h-[200px] lg:h-auto rounded-xl shadow-[10px_10px_10px_rgba(0,0,0,0.4)]' alt="" />
        <div className="flex flex-col justify-around text-white">
            <h1 className='m-2 lg:text-4xl text-2xl lg:w-full w-[310.4px] lg:h-auto h-[96px] flex   lg:font-bold font-medium '>{name}</h1>
            <p className='m-2 lg:text-2xl text-xl font-medium'>{type}</p>
            <h3 className='m-2 lg:text-2xl text-lg '>{avg_rating}/10</h3>
            <p className='m-2 lg:m-0 lg:h-fit lg:w-fit h-[144px] w-[310px] text-ellipsis overflow-hidden'>{desc}</p>
            <div className="flex justify-center lg:justify-start lg:mt-2">
                ...
                <Link
              className="hover:bg-gradient-to-r text-white from-[#be51ca] to-[#753eb9] inline m-2 ml-3 py-2.5 lg:px-6 px-3 rounded-3xl bg-gradient-to-r hover:from-[#a448ae] hover:to-[#5c3290]"
              onClick={() => toggleWishlist()}
            >
              <div className='flex justify-center items-center p-0 m-0 h-fit w-fit'>
              <span className={"text-xl m-0 h-fit w-fit p-0 flex items-center text-red-400"}>
